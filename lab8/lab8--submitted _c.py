...
def Song_str(s: Song) -> str:
    ''' Takes a song and returns the song's information as a string.
    '''
    format_str = "{:2}    {:34}    {:3} seconds long, has been played {:2} times"
    return format_str.format(s.track, s.title, s.length, s.play_count)
assert(Song_str(MUSIC[0].songs[0]) == " 1    Darkness                              411 seconds long, has been played  5 times")

def Album_str(a: Album) -> str:
    ''' Takes an album and returns the album's information as a string.
    '''
    format_str = "{:2}    {:20}    {:25} made in {:4}"
    songs = ''
    for song in a.songs:
        songs += "   " + (Song_str(song))+"\n"
    return(format_str.format(a.id, a.artist, a.title, a.year) + "\n"
          + songs)
print(Album_str(MUSIC[4]))
print(Album_str(MUSIC[3]))

def Songdisplay_str(sd: Songdisplay) -> str:
    ''' Takes a Songdisplay and returns the Songdisplay as a string.
    '''
    format_str = "{:20}  {:25}  {:4}  {:2}  {:34}  {:3}  {:2}"
    return format_str.format(sd.a_title,sd.artist,sd.year,sd.track,sd.s_title,sd.length,sd.play_count)
print(Songdisplay_str(test_list[0]))

# c.2)

def Album_trackamount(A: Album) -> int:
    ''' Returns the amount of tracks in the album.
    '''
    trackamount = len(A.songs)
    return trackamount
MUSIC.sort(key=Album_trackamount)

print("\n" + "Albums sorted by amount of tracks, lowest to highest" + "\n")
for album in MUSIC:
    print(Album_str(album))

print("Resorted with the collection_sort function" + "\n")
MUSIC.sort(key=Album_title)
collection_sort(MUSIC, Album_trackamount)
for album in MUSIC:
    print(Album_str(album))
...
def unplayed_songs(MC: 'list of Album') -> list:
    ''' Takes a music collection and returns a list of Songdisplays, one for each song that has never been played.
    '''
    list_of_Songdisplays = all_Songdisplays(MC)
    result = []
...
print("Songdisplay_str() used on unplayed songs in MUSIC" + "\n")
...
def length_from_songdisplay(sd:Songdisplay) -> int:
    ''' Returns the length of a Songdisplay.
    '''
    return sd.length
...
def Song_listening_time(s: Song) -> int:
    ''' Returns the total amount of seconds that the song has been listened to.
    '''
    return(s.length * s.play_count)

def Album_listening_time(a: Album) -> int:
    ''' Returns the total amount of seconds that the album has been listened to.
    '''
    album_totaltime = 0
    for song in a.songs:
        album_totaltime += Song_listening_time(song)
    return album_totaltime

def favorite_album(MC: 'list of Album') -> Album:
    ''' Takes a list of albums and returns the album that has been played the most and is longest.
    '''
    collection_sort(MC, Album_listening_time)
    return(MC[-1])

print("\n" + "Favorite album in MUSIC" + "\n")
print(Album_str(favorite_album(MUSIC)))

# c.6)

def top_n(MC: 'list of Album', n: int, keyfunction: "Function on Songdisplay", b: bool) -> 'list of Songdisplay':
    ''' Returns the lowest or highest n songs in MC for a keyfunction on Songdisplays
    '''
    list_of_Songdisplays = all_Songdisplays(MC)
    list_of_Songdisplays.sort(key=keyfunction, reverse=b)
    return list_of_Songdisplays[:n]

# print("\n" + "Most or Least of something in a collection" + "\n")
...
def favorite_album2(MC: 'list of Album', keyfunction: "Function on Albums") -> Album:
    ''' Takes a list of albums and returns the album that has the most of largest of a specified value.
    '''
    collection_sort(MC, keyfunction)
    return(MC[-1])

assert(Album_str(favorite_album(MUSIC)) == Album_str(favorite_album2(MUSIC, Album_listening_time)))
print("\n" + "Favorite album in MUSIC" + "\n")
print(Album_str(favorite_album2(MUSIC, Album_trackamount)))

# c.8)

def collection_search(MC: 'list of Album', s: str) -> 'list of Songdisplay':
    ''' Takes a collection and a string and returns a list of Songdsiplays of
    songs whose title, artists, or album titles include that string. '''
    list_of_Songdisplays = all_Songdisplays(MC)
    result = [ ]
    for sd in list_of_Songdisplays:
        if s in sd.artist or s in sd.a_title or s in sd.s_title:
            result.append(sd)
    return result
# print(collection_search(MUSIC, "I"))
